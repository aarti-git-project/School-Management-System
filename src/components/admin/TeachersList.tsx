...
export function TeachersList({ isDarkMode }: TeachersListProps) {
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className={`p-3 rounded-lg ${isDarkMode ? 'bg-gray-700' : 'bg-blue-100'}`}>
          <GraduationCap className={`w-6 h-6 ${isDarkMode ? 'text-gray-300' : 'text-blue-600'}`} />
        </div>
      </div>
            ...
            <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
              {teachers.map((teacher, index) => (
                <tr key={index} className={isDarkMode ? 'text-gray-300' : 'text-gray-700'}>
                  ...
                  <td className="p-4">{teacher.grade}</td>
                  <td className="p-4">
